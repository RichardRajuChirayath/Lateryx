...
if [ "$OUTPUT_FORMAT" = "markdown" ] || [ "$OUTPUT_FORMAT" = "both" ]; then
    ...
    cat > "$RESULT_MD" << EOF
# 🔒 Lateryx Security Analysis Report

## Summary
$(if [ "$IS_SAFE" = "true" ]; then echo "✅ **SAFE** - No Causality Breaches detected"; else echo "⚠️ **BREACHES DETECTED** - Infrastructure changes introduce security risks"; fi)

| Metric | Value |
|--------|-------|
| Breaches Detected | $BREACHES_COUNT |
| New Attack Paths | $NEW_PATHS |
| Shortened Paths | $SHORTENED_PATHS |
| Highest Severity | $HIGHEST_SEVERITY |

EOF
    
    if [ "$BREACHES_COUNT" -gt 0 ]; then
        echo "## Detected Breaches" >> "$RESULT_MD"
        echo "" >> "$RESULT_MD"
        
        jq -r '.breaches[] | "### [\(.severity)] \(.breach_type)\n\n**Description:** \(.description)\n\n**Remediation:** \(.remediation)\n\n---\n"' "$RESULT_JSON" >> "$RESULT_MD"
    fi
